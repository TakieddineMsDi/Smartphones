			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-briefcase"></i> Actualitites</h2>
						<div class="box-icon">
						<a class="btn" href="#" onclick='javascript:dorefresh()'><i class="icon-refresh"></i></a>
							<a href="AdminPanel.Do?addactualite=1" class="btn btn-round"><i class="icon-plus"></i></a>
					...
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <core:set var="actualites" value="<%=admin.get(\"actualites\", \"\", \"\", \"\", \"\", \"Date\", \"DESC\", \"\", \"List\") %>"></core:set>
						  <core:if test="${actualites!=null&&!actualites.get(0).isEmpty() }">
						  <core:forEach var="actualite" items="${actualites }">
							<tr id="${actualite.getID() }" name="${actualite.getID()}">
							    <td style="visibility: collapse;display: none;">${actualite.getID()}</td>
								<td>
								<li class="thumbnail">
								<a style="background:url(img/gallery/thumbs/1.jpg)" title="" href="././assets/images/ActualitesImages/${actualite.getURL()}"><img class="grayscale" src="././assets/images/ActualitesImages/${actualite.getURL()}" alt="${actualite.getTitre()}"></a>
							    </li>
							    </td>
								...
								<td class="center">
									${actualite.getOwner().getUsername()}
								</td>
								<td class="center">
								<core:if test="${actualite.isActive()}">
								   <span class="label label-success">Actif</span>
								   </core:if>
									<core:if test="${actualite.isInactive()}">
								   <span class="label label-warning">Inactif</span>
								   </core:if>
								   <core:if test="${actualite.isArchive()}">
								   <span class="label label-info">Archived</span>
								   </core:if>
								   <core:if test="${actualite.isBanned()}">
								   <span class="label label-important">Banned</span>
								   </core:if>
								</td>
								<td class="center">
								<div class="btn-group">
								  <a class="btn btn-primary" href="#"><i class="icon-user icon-white"></i> actualite</a>
								  <a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
								  <ul class="dropdown-menu">
									<li><a href="AdminPanel.Do?editactualite=${actualite.getID()}"><i class="icon-pencil"></i> Edit</a></li>
									<li><a href="#" onclick='javascript:dothis("deleteactualite","${actualite.getID()}")'><i class="icon-trash"></i> Delete</a></li>
							    <core:if test="${actualite.isInactive()}">
								<li><a href="#" onclick='javascript:dothis("activeactualite","${actualite.getID()}")'><i class="icon-ok-circle"></i> Activate</a></li>
								</core:if>
								<core:if test="${actualite.isArchive()}">
							    <li><a href="#" onclick='javascript:dothis("restoreactualite","${actualite.getID()}")'><i class="icon-ban-circle"></i> restore</a></li>
							    </core:if>
							    <core:if test="${!actualite.isArchive()}">
							    <li><a href="#" onclick='javascript:dothis("archiveactualite","${actualite.getID()}")'><i class="icon-ok-circle"></i> Archive</a></li>
							   </core:if>
							   <core:if test="${!actualite.isBanned()}">
								<li><a href="#" onclick='javascript:dothis("banactualite","${actualite.getID()}")'><i class="icon-check"></i> Ban</a></li>
								</core:if>
								<core:if test="${actualite.isBanned()}">
								<li><a href="#" onclick='javascript:dothis("unbanactualite","${actualite.getID()}")'><i class="icon-remove-circle"></i> Unban</a></li>
								</core:if>
								  </ul>
								</div>
								</td>
...
function update(action,id) {
	...
	self.xmlHttpReq.send("actionactualite="+action+","+id);
	}
...
function refresh() {
	...
	self.xmlHttpReq.send("refreshactualite=1");
	}
