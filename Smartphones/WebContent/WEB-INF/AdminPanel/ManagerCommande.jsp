			...
			<div class="span2 main-menu-span">
				<div class="well nav-collapse sidebar-nav">
					<ul class="nav nav-tabs nav-stacked main-menu">
						...
						<li><a class="ajax-link" href="AdminPanel.ManagerActualite"><i class="icon-list-alt"></i><span class="hidden-tablet"> Manage Actualites</span></a></li>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-briefcase"></i> Commandes</h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <core:set var="commandes" value="<%=admin.get(\"commandes\", \"\", \"\", \"\", \"\", \"Date\", \"DESC\", \"\", \"List\") %>"></core:set>
						  <core:if test="${commandes!=null&&!commandes.get(0).isEmpty() }">
						  <core:forEach var="commande" items="${commandes }">
							 <tr id="${commande.getID() }" name="${commande.getID()}">
							    <td style="visibility: collapse;display: none;">${commande.getID()}</td>
								<td>
								<li class="thumbnail">
								<a style="background:url(img/gallery/thumbs/1.jpg)" title="" href="././assets/images/ProductImages/${commande.getProduit().getURL()}"><img class="grayscale" src="././assets/images/ProductImages/${commande.getProduit().getURL()}" alt="${commande.getProduit().getLibelle()}"></a>
							    </li>
							    </td>
								...
								<td class="center">
								<core:if test="${commande.isValidated()}">
								<span class="label label-success">${commande.getValidator().getUsername()}</span>
								</core:if>
								<core:if test="${!commande.isValidated()}">
								<input type="text" class="add-on input-small datepicker" id="v${commande.getID()}" value="Date">
								</core:if>
								</td>
							<td class="center">
								<core:if test="${commande.isActive()}">
								   <span class="label label-success">Actif</span>
								   </core:if>
									<core:if test="${commande.isInactive()}">
								   <span class="label label-warning">Inactif</span>
								   </core:if>
								   <core:if test="${commande.isArchive()}">
								   <span class="label label-info">Archived</span>
								   </core:if>
								   <core:if test="${commande.isBanned()}">
								   <span class="label label-important">Banned</span>
								   </core:if>
								</td>
								<td class="center">
								<div class="btn-group">
								  <a class="btn btn-primary" href="#"><i class="icon-shopping-cart icon-white"></i> commande</a>
								  <a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
								  <ul class="dropdown-menu">
								  <core:if test="${!commande.isValidated()}">
								  <li><a href="#" onclick='javascript:dothis("validecommande","${commande.getID()}")'><i class="icon-pencil"></i> Validate</a></li>
									</core:if>
									<li><a href="AdminPanel.Do?editcommande=${commande.getID()}"><i class="icon-pencil"></i> Edit</a></li>
									<li><a href="#" onclick='javascript:dothis("deletecommande","${commande.getID()}")'><i class="icon-trash"></i> Delete</a></li>
								<core:if test="${commande.isInactive()}">
								<li><a href="#" onclick='javascript:dothis("activecommande","${commande.getID()}")'><i class="icon-ok-circle"></i> Activate</a></li>
								</core:if>
								<core:if test="${commande.isArchive()}">
							    <li><a href="#" onclick='javascript:dothis("restorecommande","${commande.getID()}")'><i class="icon-ban-circle"></i> restore</a></li>
							    </core:if>
							    <core:if test="${!commande.isArchive()}">
							    <li><a href="#" onclick='javascript:dothis("archivecommande","${commande.getID()}")'><i class="icon-ok-circle"></i> Archive</a></li>
							   </core:if>
							   <core:if test="${!commande.isBanned()}">
								<li><a href="#" onclick='javascript:dothis("bancommande","${commande.getID()}")'><i class="icon-check"></i> Ban</a></li>
								</core:if>
								<core:if test="${commande.isBanned()}">
								<li><a href="#" onclick='javascript:dothis("unbancommande","${commande.getID()}")'><i class="icon-remove-circle"></i> Unban</a></li>
								</core:if>
								  </ul>
								</div>
								</td>
...
function update(action,id) {
	...
	if(action=="validecommande")
		{
		self.xmlHttpReq.send("actioncommande="+action+","+id+","+document.getElementById("v"+id).value);
		}
	else{
		self.xmlHttpReq.send("actioncommande="+action+","+id);
	}
	}
...
function refresh() {
	...
	self.xmlHttpReq.send("refreshcommande=1");
	}
